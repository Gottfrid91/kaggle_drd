...
def train():
    ...
    with tf.Graph().as_default():
        ...
        #
        balanced = drd.balanced_indicator()
        unbalanced = drd.unbalanced_indicator()
        # Get images and labels
        images, labels, names, dir = drd.distorted_inputs(balanced,unbalanced)
        #get validation data
        val_images, val_labels = drd.inputs(True, balanced, unbalanced)
        ...
        logits = drd.inference_2Blocks(images, keep_prob)
        val_logits = drd.inference_2Blocks(val_images, keep_prob)
        ...
        for step in range(step_start, FLAGS.max_steps):
            ...
            #set step conditions to regulate which data set is beeing training on
            if step == 10:
                print("changing data set to balanced data")
                balanced = sess.run([balanced])

            if step == 60:
                print("changing back to regular data")
                unbalanced = sess.run([unbalanced])
