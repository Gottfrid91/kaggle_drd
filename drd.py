...
import argparse
import re
from resnet_model_offic import *
...
parser.add_argument('--train_dir', type=str,
                    default='/media/olle/Seagate Expansion Drive/DRD_master_thesis_olle_holmberg/kaggle/train_zip_files/records/',
...
def distorted_inputs():
    ...
    images, labels = drd_input.distorted_inputs(data_dir=FLAGS.train_dir,batch_size=FLAGS.batch_size)
    if FLAGS.use_fp16:
...
def inputs(eval_data):
    ...
    Raises:
      ValueError: If no data_dir
    """
    images, labels = drd_input.inputs(eval_data=eval_data,
...
def test_inputs(data_dir, batch_size):
    ...
    Raises:
      ValueError: If no data_dir
    """
    images, labels = drd_input.test_inputs(data_dir = data_dir)
    if FLAGS.use_fp16:
        images = tf.cast(images, tf.float16)
        labels = tf.cast(labels, tf.float16)
    return images, labels
...
def resnet_v1_50(images, training):
    return(rm.resnet_v1_4Blocks(images, num_classes=5,is_training = training, scope="resnet_v1_50", reuse=tf.AUTO_REUSE))
...
def loss(logits, labels):
  ...
  weights_initializer = tf.constant_initializer(value=[1.28205128, 1200.5, 1100.11111111, 3300.33333333, 5000.])
  ...
  #     name="class_weighing"
  # )
  labels = tf.one_hot(tf.cast(labels, tf.int64), 5)
  cross_entropy = tf.nn.weighted_cross_entropy_with_logits(targets=labels, logits=tf.nn.softmax(logits), pos_weight=class_weights)
...
def train(total_loss, global_step):
  ...
  with tf.control_dependencies([variable_averages,apply_gradient_op]):
