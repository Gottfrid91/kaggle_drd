...
parser.add_argument('--train_dir', type=str,
                    default='/home/olle/PycharmProjects/Diabetic_Retinopathy_Detection/create_tfrecords/train/',
                    help='directory where training data is stored')
# Basic model parameters.
parser.add_argument('--validation_dir', type=str,
                    default='/home/olle/PycharmProjects/Diabetic_Retinopathy_Detection/create_tfrecords/validation',
                    help='directory where training data is stored')
# Basic model parameters.
...
def balanced_indicator():
    #data set indicator
    balanced = tf.Variable(1, trainable=False, name='data_set_indicator')
    return(balanced)
def unbalanced_indicator():
    # data set indicator
    balanced = tf.Variable(0, trainable=False, name='data_set_indicator')
    return (balanced)

def distorted_inputs(balanced, unbalanced):
    ...
    #below we change the data source based on training step. see drd_trian_resnet_v1_30
    dir = FLAGS.train_dir
    #balanced is the boolean indicated depicting if the balanced data set should be trained on
    if balanced == 1:
        dir = FLAGS.balanced_train_dir
    images, labels, names = drd_input.distorted_inputs(data_dir=dir,batch_size=FLAGS.batch_size)
    print(dir)
    if FLAGS.use_fp16:
        images = tf.cast(images, tf.float16)
        labels = tf.cast(labels, tf.float16)
    return images, labels, names, dir

def inputs(eval_data, balanced, unbalanced):
    ...
    """
    #below we change the data source based on training step. see drd_trian_resnet_v1_30
    val_dir = FLAGS.validation_dir
    #balanced is the boolean indicated depicting if the balanced data set should be trained on
    if balanced == True:
        val_dir = FLAGS.balanced_validation_dir
    print(val_dir)
    images, labels, names = drd_input.inputs(eval_data=eval_data,
                                       batch_size=FLAGS.batch_size, data_dir = val_dir)
    if FLAGS.use_fp16:
        images = tf.cast(images, tf.float16)
        labels = tf.cast(labels, tf.float16)
...
def inference_2Blocks(images, keep_prob):
    ...
    with tf.variable_scope('x', reuse=tf.AUTO_REUSE) as scope:
        ...
        #applying drop out
        local_3_drop = tf.nn.dropout(local3, keep_prob)  # DROP-OUT here
        _activation_summary(local_3_drop)

    # local4
    with tf.variable_scope('y', reuse=tf.AUTO_REUSE) as scope:
        weights = _variable_with_weight_decay('weights', shape=[384, 192],
                                              stddev=0.04, wd=0.004)
        biases = _variable_on_cpu('biases', [192], tf.constant_initializer(0.1))
        local4 = tf.nn.relu(tf.matmul(local_3_drop, weights) + biases, name=scope.name)
        local_4_drop = tf.nn.dropout(local4, keep_prob)  # DROP-OUT here
        _activation_summary(local_4_drop)
    ...
    with tf.variable_scope('softmax_linear', reuse=tf.AUTO_REUSE) as scope:
        ...
        softmax_linear = tf.add(tf.matmul(local_4_drop, weights), biases, name=scope.name)
